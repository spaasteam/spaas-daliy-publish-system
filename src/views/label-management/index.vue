<template>
  <div class="label-management">
    ...
    <loading-dialog :visible.sync="dialogVisible" :title="labelTitle" width="500px" :confirm="handleValidateForm">
...
import { getlabelList, postLabel, deleteLabel, patchLabel } from "@/services/v1/github";
  ...
  setup(props, ctx) {
    ...
    const handleDelete = (row) => {
      $loadingConfirm({
        title: '删除提示',
        text: '确定要删除该标签吗?',
        confirm: () => deleteLabel(row.name).then(() => {
          toast('操作成功')
          getList()
        })
      })
    ...
    const handleValidateForm = async () => {
      ...
      const promise = labelEdit.value ? patchLabel({
        ...params,
        currentName: oldName
      }) : postLabel(params)
    ...
    const labelTitle = computed(() => labelEdit.value ? '编辑标签' : '新增标签')
